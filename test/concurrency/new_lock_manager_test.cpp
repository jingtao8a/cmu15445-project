...
void CheckShrinking(Transaction *txn) {
  // LOG_DEBUG("txn:%d CheckShrinking %d", txn->GetTransactionId(), txn->GetState());
...
TEST(LockManagerTest, MixedTest) {
  ...
  fmt::print("------\n");
  fmt::print("txn1_id:{} txn2_id:{}", txn1->GetTransactionId(), txn2->GetTransactionId());
  query = "delete from test_1 where x = 100;";
  fmt::print("------ txn2 1 begin\n");
  bustub->ExecuteSqlTxn(query, writer, txn2);
  fmt::print("------ txn2 1 end\n");

  query = "select * from test_1;";
  fmt::print("------ txn2 2 begin\n");
  bustub->ExecuteSqlTxn(query, writer, txn2);
  fmt::print("------ txn2 2 end\n");
